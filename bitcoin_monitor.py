...
import pytz
from config import TELEGRAM_TOKEN, CHAT_ID, TARGET_ADDRESS, MAX_RECONNECT_ATTEMPTS, TIMEZONE
from monitor.notifier import send_telegram_message
from monitor.blockchain import get_btc_to_usd
from utils.value_utils import get_usd_value, format_btc_amount
from utils.time_utils import format_time

# Variables para manejar la reconexión y la última transacción
...
def on_close(ws, close_status_code, close_msg):
    ...
    if reconnect_attempts <= MAX_RECONNECT_ATTEMPTS:
        time.sleep(5)
        send_telegram_message("Reconectando...")
        connect()
    else:
...
def on_open(ws):
    ...
    # Obtener la última transacción de la dirección al abrir la conexión
